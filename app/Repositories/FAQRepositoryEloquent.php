...
class FAQRepositoryEloquent implements FAQRepository {
	 ...
	 * @param Status $status
	 */
	public function __construct(FAQ $model, Status $status) {
		$this->model  = $model;
		$this->status = $status;
	}
	...
	public function all($columns = ['*']) {
		try {
			$res = Cache::remember('faqs.all', Config::get('cache.duration.hour'), function() use($columns) {
	...
	public function find($id, $columns = ['*']) {
		try {
			return Cache::remember('faqs.find.' . $id, Config::get('cache.duration.hour'), function() use($id, $columns) {
	...
	public function findBy($key, $val, $columns = ['*']) {
		try {
			return Cache::remember('faqs.findBy.' . $key . '.' . str_slug($val), Config::get('cache.duration.hour'), function() use($key, $val, $columns) {
	...
	public function save($input = []) {
		try {
			...
			$response['message'] = 'FAQ has been saved.';
		} catch (ModelNotFoundException $e) {
	...
	public function delete($id, $force = FALSE) {
		try {
			...
			if ($force) {
				...
				throw new Exception('Error deleting FAQ: ' . $id);
			}
	...
	public function selectList($key, $val) {
		try {
			$res = Cache::remember('faqs.selectList.' . $key . '.' . $val, Config::get('cache.duration.hour'), function() use($val) {
	...
	public function filteredList($input = []) {
		try {
			...
			// Build query
			$query = $this->model
				->select('faqs.id', 'faqs.title AS title', 'statuses.status AS status')
	...
	public function setValue($key, $val, $id) {
		try {
			...
			$this->model->{$key} = $val;
			if (!$this->model->save()) {
				throw new Exception('Unable to update FAQ, please try again.');
			}
			...
			// Success
			$response['success'] = TRUE;
			$response['message'] = 'FAQ has been updated.';
		} catch (ModelNotFoundException $e) {
			$response['error'] = 'Record not found.';
		} catch (Exception $e) {
			log::error($e);
	...
	public function statusList() {
		try {
			$status = $this->status;
			$res    = Cache::remember('faqs.statusList', Config::get('cache.duration.hour'), function() use($status) {
